...
| GET | `/b/{board}/cards/{id}` | Card detail: notes, tag, checklist, blocked, meta, and the card's lane for `J`/`K` | `enter` detail screen |
...
| GET | `/static/lanes.js` | The board page's phone layout, one lane at a time: keeps the lane tabs and the add button in step with the lane scrolled into view, and reopens on the lane last shown. Loaded from the shared footer and a no-op off the board. Posts nothing | — (plumbing) |
| GET | `/static/edit.js` | The card page's inline editing: submits the page's existing forms when a changed field is left, on Enter, or on ⌘S/Ctrl+S in the notes, and binds the detail screen's keys (`esc`, `T`, `t`, `e`, `o`, `b`, `m`, `d`, `J`/`K`) to those same forms and links. Loaded in `<head>` so its `js` class is set before first paint | — (plumbing) |
...
`/b/{board}/events` and the four `/static/*.js` files are the non-form
routes. The listener (KD-1, §4) is a small vanilla-JS file that reacts to the
stream by reloading the current page; the drag script is the other case KD-1
allows, and it ends in a form `POST` to a route in this table rather than a
write of its own. `lanes.js` only scrolls and relabels, and `edit.js` only
decides when an existing form submits, so neither adds a way to write a card.
All four are served rather than inlined so the Content-Security-Policy need
not allow inline script, and each is pinned to its own path so the embed is
never widened to a pattern.
