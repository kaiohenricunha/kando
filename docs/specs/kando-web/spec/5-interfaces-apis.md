...
| POST | `/b/{board}/cards/{id}/move` | Move to a lane (`lane` param), optionally to a position in it: `pos=before` or `pos=after` with `anchor=<card id>`, or `pos=start` for a lane rendering no cards. No `pos` lands the card on top, as the lane picker always has. A position is named against a card the user can see, never an index — the page filters, so the last card on screen is not the last in the lane — and an `anchor` that has left the destination lane is a 409; an unknown `pos`, or one with no `anchor`, is a 400 | `H`/`L`, `d`, `m` picker; the position is drag-and-drop only (BOUND-1b, §2) |
...
| GET | `/static/dnd.js` | The board page's drag-and-drop, served from this origin for the same reason. A drop builds and posts a form at the `/move` route above, so there is no second way to write a card | — (plumbing) |
