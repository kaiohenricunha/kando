...
Reached with `D` (or via the Done lane). Header breadcrumb `kando  life › archive`, right `N done`. The cursor always sits on a listed row: when the clock carries a card past an `age` filter, it moves onto the last row left. Content max 80 cols: groups `THIS WEEK 3`, `LAST WEEK 4`, `EARLIER 3` (muted uppercase, count, underline in `border`), each item one row: `✓` (accent) · title (fills, truncates) · tag (accent, 8 cols) · date right-aligned 10 cols muted. Blank row between groups. Trailing note: `Older entries live in ~/.kando/life/archive.md` (path in fg).
