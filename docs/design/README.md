...
3. Rows 3–37 lanes.
4. Row 38 blank.
5. Row 39 footer key hints; right-aligned card count `11 cards`. When a save or the file watcher fails, `⊘ message` in `accent2` takes the count's place until the next successful write. When a read fails — a reload, or an `archive.md` that cannot be read — the next reload that reads the file clears it too. When a key is refused, its reason takes that slot until the next key, and a standing failure shows again after it. Either way the message gets at most half the row, and the hints give way rather than the message. The detail and archive footers gain the same slot at their right end.
